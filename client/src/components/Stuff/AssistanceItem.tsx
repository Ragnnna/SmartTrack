...
const AssistanceItem: React.FC<AssistanceItemProps> = (props) => {
  ...
  return (
    <div className="doctor-block">
      <div className="index-doctor assistance list-element"><p>{idx + 1}</p></div>
      <div className="doctor-name list-element list-element-title-assistance"><p className="list-title-name">{ name }</p></div>
      <div className="doctor-email list-element list-element-title-assistance"><p>{ email }</p></div>
      <div className="doctor-number list-element list-element-title-assistance"><p>{ phone }</p></div>
      { context.getUserType() === 'admin' &&
      <div className="assistance-interface">
        <div className="interface-element"><span className="material-icons">edit_calendar</span></div>
      ...
      {deleteModal && <DeleteUserAccept 
          deleteDoctor={props.deleteAssistante} setDeleteModal={setDeleteModal} 
          id={id}
        />
      }
    </div>
